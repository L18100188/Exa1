...
class TercerFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val swDarkMode = view.findViewById<Switch>(R.id.darkmode)

        swDarkMode.setOnCheckedChangeListener{_, isSelected ->
            if(isSelected)
            {
                enableDM()
                Toast.makeText(activity, "Se hizo de noche!", Toast.LENGTH_SHORT).show()
            }
            else
            {
                disableDM()
                Toast.makeText(activity, "Se hizo de Dia!", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun enableDM()
    {
        AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_YES)
    }

    private  fun disableDM(){
